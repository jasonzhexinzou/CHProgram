            ...
            this.$menu.on('click', 'li a', function (e) {
                ...
                if (!that.isDisabled() && !$this.parent().hasClass('disabled')) {
                    ...
                    if ((prevValue != that.$element.val() && that.multiple) || (prevIndex != that.$element.prop('selectedIndex') && !that.multiple)) {
                        //selected other options then select all
                        if (prevValue != null && prevValue[0] != "ALL" && that.$element.val() != null && (that.$element.val())[0] == "ALL" && that.multiple) {
                            that.$menu.find('.selected').removeClass('selected');
                            $options.prop('selected', false);
                            $option.prop('selected', !state);
                            that.setSelected(clickedIndex, !state);
                            ...
                            //selected all then select other options
                        else if (prevValue != null && prevValue[0] == "ALL" && that.$element.val() != null && that.$element.val().length > 1 && that.multiple) {
                            that.$menu.find('.selected').removeClass('selected');
                            $options.prop('selected', false);
                            $option.prop('selected', !state);
                            that.setSelected(clickedIndex, !state);
