$(function () {
    ...
    $(".win-favorite").click(function () {
        ...
        try { window.external.addFavorite(sURL, sTitle); }
            ...
            catch (exp) { alert("加入收藏失败，请使用Ctrl+D进行添加"); }
        }
    });
    ...
    $showdialog = function (e) {
        ...
        var title = e.title == '' || e.title == null || e.title == undefined ? '提示' : e.title;
        var body = e.body;
        ...
        detail += '</div>';
        detail += '<div class="dialog-body" style="overflow-y:scroll;max-height: 300px;">';
        detail += body;
        detail += '</div>';
        detail += '<div class="dialog-foot">';
        // detail += '  <button class="button dialog-close">取消</button>';
        detail += '  <button class="button bg-green btn_ok">确认</button>';
        detail += '</div>';
        detail += '</div>';
        detail += '</div>';
...
/**
* 从下面开始，是Master.Jiang扩展的拼图Ajax方法
*/
...
jQuery.fn.pintuerBooleanRadio = function (val) {
    ...
    if (!this.hasClass('radio')) {
        this.pintuerRadio('init', {
            items: [
                    { val: 1, cls: 'icon-check text-green', txt: '是' },
                    { val: 0, cls: 'icon-times', txt: '否' }
            ]
        });
    }
...
// 由于忘记有什么用，并且这代码起到不好的作用，所以注释掉了
///**
//* 抄来的输入框光标控制扩展
//* 用法：读位置 alert($(this).position());  设置位置： $(this).position(4);
//*/
//$.fn.extend({
